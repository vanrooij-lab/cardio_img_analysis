...
my_work_dir = '/Volumes/Wehrens_Mic/IMG_ANALYSIS/2023-03-17_Eloi_ISO_EPI_CM_96wells/'
...
sample_info  = pd.read_excel(my_work_dir+'2023-03-17_eloi_metadata.xlsx')
...
# Select all samples
SELECTED_SAMPLES = sample_info['sample_name']
...
SEARCHTERMS = ['NegControle','-EPI$','-ISO$','-EPI.ISO$']
mycolors    = ['b'  ,'r'  ,'r'        ,'r'  ]
...
# Print in slightly more convenient format
for k in peak_durations.keys():
    print(k+"\t"+ str(peak_durations[k]))
    
for k in interpeak_times.keys():
    print(k+"\t"+ str(interpeak_times[k]))
